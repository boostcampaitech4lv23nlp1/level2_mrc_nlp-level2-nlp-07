...
class MRC_Dataset(torch.utils.data.Dataset):
    """ Dataset 구성을 위한 class."""
    def __init__(self, dataset,tokenizer,cfg):
        try:
            self.labels = [{"id" : ex['id'], 'answers' : ex['answers']} for ex in dataset]
        except:
            self.labels = None
    ...
    def __getitem__(self, idx):
        if self.labels ==None:
            return {k : v for k,v in self.dataset[idx].items()}
        else:
            sample = {k : v for k,v in self.dataset[idx].items()}
            return sample
    def __len__(self):
    ...
    def prepare_features(self,examples):
        ...
        tokenized_examples = self.tokenizer(
            ...
            max_length= self.cfg.data.max_seq_length, # self.cfg.max_seq_length,
    ...
    def prepare_test_features(self,examples):
        ...
        tokenized_examples = self.tokenizer(
            ...
            stride= self.cfg.data.doc_stride,
            return_overflowing_tokens=True,
            return_offsets_mapping=True,
            return_token_type_ids=self.cfg.is_not_roberta, # roberta모델을 사용할 경우 False, bert를 사용할 경우 True로 표기해야합니다.
            padding="max_length"
        )
        tokenized_examples["example_id"] = []
        sample_mapping = tokenized_examples.pop("overflow_to_sample_mapping")
    ...
    def post_processing_function(self, examples, features, predictions, training_args):
        ...
        predictions = postprocess_qa_predictions(
            ...
            max_answer_length=self.cfg.data.max_answer_length,
            output_dir=training_args.output_dir,
            prefix = self.cfg.exp.prediction_prefix
        )
