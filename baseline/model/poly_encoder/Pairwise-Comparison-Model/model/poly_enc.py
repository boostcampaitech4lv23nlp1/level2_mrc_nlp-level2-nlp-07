...
from model.basic import PCModel
...
class PolyEncoder(PCModel):
    def __init__(self, args, model_name, pooling_method, n_codes=64):
        ...
        codes = torch.nn.init.xavier_uniform_(codes, gain=1.0)
        self.codes = torch.nn.Parameter(codes)
        # self.codes = torch.nn.Linear(self.n_codes, args.embed_size)
    ...
    def forward(self, query, cand):
        ...
        h = self.context_encoder(query.repeat(batch_size,1))
